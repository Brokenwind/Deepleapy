...
def squared_loss(y_true, y_pred, **args):
...
def softmax_loss(y_true, y_prob, axis=0, clip=True):
    ...
    return -np.sum(y_true * np.log(y_prob)) / y_prob.shape[axis]

def log_loss(y_true, y_prob, axis=0, clip=True):
    ...
    res = -np.sum(y_true * np.log(y_prob) +
                  (1 - y_true) * np.log(1 - y_prob)) / y_prob.shape[axis]
    return res
...
def compute_cnn_cost(hyperparams, y_true, y_prob):
    """
    X: the input test data
    y: the label of relative x
    reg: if it is True, means using regularized logistic. Default False
